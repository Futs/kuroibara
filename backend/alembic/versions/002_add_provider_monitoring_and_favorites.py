...
def upgrade() -> None:
    ...
    if "provider_status" not in inspector.get_table_names():
        op.create_table(
            ...
            sa.Column(
                "successful_checks", sa.Integer(), nullable=False, server_default="0"
            ),
            sa.Column(
                "uptime_percentage", sa.Integer(), nullable=False, server_default="100"
            ),
            sa.Column(
    ...
    if table_created or "provider_status" in inspector.get_table_names():
        existing_indexes = [
            idx["name"] for idx in inspector.get_indexes("provider_status")
        ]
        ...
        if "ix_provider_status_status" not in existing_indexes:
            op.create_index(
                op.f("ix_provider_status_status"), "provider_status", ["status"]
            )

        if "ix_provider_status_is_enabled" not in existing_indexes:
            op.create_index(
                op.f("ix_provider_status_is_enabled"), "provider_status", ["is_enabled"]
            )

        if "ix_provider_status_last_check" not in existing_indexes:
            op.create_index(
                op.f("ix_provider_status_last_check"), "provider_status", ["last_check"]
            )
...
def downgrade() -> None:
    ...
    if "provider_status" in inspector.get_table_names():
        ...
        if "ix_provider_status_last_check" in existing_indexes:
            op.drop_index(
                op.f("ix_provider_status_last_check"), table_name="provider_status"
            )

        if "ix_provider_status_is_enabled" in existing_indexes:
            op.drop_index(
                op.f("ix_provider_status_is_enabled"), table_name="provider_status"
            )

        if "ix_provider_status_status" in existing_indexes:
            op.drop_index(
                op.f("ix_provider_status_status"), table_name="provider_status"
            )

        if "ix_provider_status_provider_id" in existing_indexes:
            op.drop_index(
                op.f("ix_provider_status_provider_id"), table_name="provider_status"
            )
