...
def upgrade() -> None:
    ...
    if "manga" in inspector.get_table_names():
        ...
        if "provider" not in existing_columns:
            op.add_column(
                "manga", sa.Column("provider", sa.String(length=50), nullable=True)
            )

        if "external_id" not in existing_columns:
            op.add_column(
                "manga", sa.Column("external_id", sa.String(length=255), nullable=True)
            )

        if "external_url" not in existing_columns:
            op.add_column(
                "manga", sa.Column("external_url", sa.String(length=500), nullable=True)
            )
        ...
        if "ix_manga_provider" not in existing_indexes:
            op.create_index(
                op.f("ix_manga_provider"), "manga", ["provider"], unique=False
            )
