...
def upgrade() -> None:
    ...
    if "user_provider_preferences" in inspector.get_table_names():
        existing_indexes = [
            idx["name"] for idx in inspector.get_indexes("user_provider_preferences")
        ]

        if "ix_user_provider_preferences_user_id" not in existing_indexes:
            op.create_index(
                op.f("ix_user_provider_preferences_user_id"),
                "user_provider_preferences",
                ["user_id"],
            )
        ...
        if "ix_user_provider_preferences_provider_id" not in existing_indexes:
            op.create_index(
                op.f("ix_user_provider_preferences_provider_id"),
                "user_provider_preferences",
                ["provider_id"],
            )
        ...
        if "ix_user_provider_preferences_is_favorite" not in existing_indexes:
            op.create_index(
                op.f("ix_user_provider_preferences_is_favorite"),
                "user_provider_preferences",
                ["is_favorite"],
            )
        ...
        if "ix_user_provider_preferences_priority_order" not in existing_indexes:
            op.create_index(
                op.f("ix_user_provider_preferences_priority_order"),
                "user_provider_preferences",
                ["priority_order"],
            )
...
def downgrade() -> None:
    ...
    if "user_provider_preferences" in inspector.get_table_names():
        ...
        if "ix_user_provider_preferences_priority_order" in existing_indexes:
            op.drop_index(
                op.f("ix_user_provider_preferences_priority_order"),
                table_name="user_provider_preferences",
            )
