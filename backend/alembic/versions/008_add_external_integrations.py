...
def upgrade() -> None:
    ...
    integration_type_enum = postgresql.ENUM(
        "anilist", "myanimelist", name="integrationtype", create_type=False
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "last_sync_status", sync_status_enum, nullable=False, default="pending"
        ),
    ...
    # Create indexes
    op.create_index(
        "ix_external_integrations_user_id", "external_integrations", ["user_id"]
    )
    op.create_index(
        "ix_external_integrations_integration_type",
        "external_integrations",
        ["integration_type"],
    )
    op.create_index(
        "ix_external_integrations_last_sync_status",
        "external_integrations",
        ["last_sync_status"],
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["integration_id"], ["external_integrations.id"], ondelete="CASCADE"
        ),
    ...
    # Create indexes
    op.create_index(
        "ix_external_manga_mappings_integration_id",
        "external_manga_mappings",
        ["integration_id"],
    )
    op.create_index(
        "ix_external_manga_mappings_manga_id", "external_manga_mappings", ["manga_id"]
    )
    op.create_index(
        "ix_external_manga_mappings_external_manga_id",
        "external_manga_mappings",
        ["external_manga_id"],
    )
    op.create_index(
        "ix_external_manga_mappings_sync_status",
        "external_manga_mappings",
        ["sync_status"],
    )
