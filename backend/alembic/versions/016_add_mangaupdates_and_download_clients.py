...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "universal_entry_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("universal_manga_entries.id"),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "implementation", sa.String(50), nullable=False
        ),  # "qbittorrent", "deluge", "sabnzb", etc.
        # Connection settings
    ...
    op.create_table(
        ...
        sa.Column(
            "download_type", sa.String(20), nullable=False
        ),  # "provider", "torrent", "nzb"
        sa.Column(
            "source_name", sa.String(100), nullable=True
        ),  # Provider/indexer name
        # Download client (for torrent/NZB)
        sa.Column(
            "client_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("download_clients.id"),
            nullable=True,
        ),
        sa.Column(
            "external_download_id", sa.String(255), nullable=True
        ),  # ID in download client
        # Download details
    ...
    # Create indexes
    op.create_index(
        "idx_universal_entries_source_indexer",
        "universal_manga_entries",
        ["source_indexer"],
    )
    op.create_index(
        "idx_universal_entries_source_id", "universal_manga_entries", ["source_id"]
    )
    op.create_index("idx_universal_entries_title", "universal_manga_entries", ["title"])
    op.create_index(
        "idx_universal_entries_confidence",
        "universal_manga_entries",
        ["confidence_score"],
    )
    op.create_index(
        "idx_universal_mappings_manga_id", "universal_manga_mappings", ["manga_id"]
    )
    op.create_index(
        "idx_universal_mappings_entry_id",
        "universal_manga_mappings",
        ["universal_entry_id"],
    )
    op.create_index(
        "idx_cross_references_entry_id",
        "cross_indexer_references",
        ["universal_entry_id"],
    )
    op.create_index(
        "idx_cross_references_indexer",
        "cross_indexer_references",
        ["reference_indexer"],
    )
    op.create_index(
        "idx_download_clients_type_enabled",
        "download_clients",
        ["client_type", "is_enabled"],
    )
    op.create_index(
        "idx_indexers_type_enabled", "indexers", ["indexer_type", "is_enabled"]
    )
