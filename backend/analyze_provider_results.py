    ...
    if "Search not working" in str(analysis["issues"]):
        if "404" in str(analysis["issues"]):
            analysis["recommendations"].append(
                "Update search URL pattern - endpoint may have changed"
            )
        elif "403" in str(analysis["issues"]):
            analysis["recommendations"].append(
                "Site may be blocking requests - try different headers or user agent"
            )
        elif "521" in str(analysis["issues"]):
            analysis["recommendations"].append(
                "Site is behind Cloudflare protection - may need special handling"
            )
        elif "429" in str(analysis["issues"]):
            analysis["recommendations"].append(
                "Rate limited - implement delays between requests"
            )

    if "Metadata extraction failed" in str(analysis["issues"]):
        analysis["recommendations"].append(
            "Update CSS selectors for metadata extraction"
        )
...
def generate_summary_report(results: Dict[str, Any]) -> str:
    ...
    # Success rate
    success_rate = (
        (len(working) / len(detailed_results)) * 100 if detailed_results else 0
    )
    partial_rate = (
        (len(partial) / len(detailed_results)) * 100 if detailed_results else 0
    )
    report.append(
        f"Success Rate: {success_rate:.1f}% fully working, {partial_rate:.1f}% partially working"
    )
    report.append("")
...
def generate_detailed_report(results: Dict[str, Any]) -> str:
    ...
    for status in ["working", "partial", "broken"]:
        ...
        for analysis in sorted(providers, key=lambda x: x["provider_name"]):
            report.append(
                f"Provider: {analysis['provider_name']} ({analysis['provider_id']})"
            )
