...
async def create_backup(
    ...
    try:
        ...
        return CreateBackupResponse(
            ...
            backup_name=request.backup_name or f"manual_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
...
async def check_for_orphaned_storage(
    ...
    """
    try:
        orphaned_manga = await storage_recovery_service.scan_storage_for_manga(current_user.id, db)
        
        return {
            "has_orphaned_files": len(orphaned_manga) > 0,
            "orphaned_count": len(orphaned_manga),
            "message": f"Found {len(orphaned_manga)} manga in storage that are not in the database" if orphaned_manga else "No orphaned files found",
