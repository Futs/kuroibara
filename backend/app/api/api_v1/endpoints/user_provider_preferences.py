    ...
    try:
        ...
        for provider_info in provider_info_list:
            ...
            if status_record:
                ...
                is_healthy = (
                    status_record.status == ProviderStatusEnum.ACTIVE.value
                    and status_record.consecutive_failures
                    < status_record.max_consecutive_failures
                    and status_record.is_enabled
                )
            ...
            provider_with_pref = ProviderWithPreference(
                ...
                last_check=(
                    status_record.last_check.isoformat()
                    if status_record and status_record.last_check
                    else None
                ),
    ...
    try:
        ...
        if existing_pref:
            ...
            new_pref = UserProviderPreference(
                user_id=current_user.id, provider_id=provider_id, **update_data
            )
            db.add(new_pref)
...
async def bulk_update_provider_preferences(
    ...
    try:
        logger.info(
            f"Bulk updating {len(bulk_update.preferences)} provider preferences for user {current_user.id}"
        )
