...
class BatchOrganizer:
                            ...
                            try:
                                ...
                                if result.success:
                                    ...
                                    history = OrganizationHistory(
                                        ...
                                        destination_path=(
                                            result.organized_files[0]
                                            if result.organized_files
                                            else None
                                        ),
                                        naming_format_used=custom_naming_format
                                ...
                                if (
                                    processed_chapters % 10 == 0
                                    or processed_chapters == total_chapters
                                ):
                                ...
                                processed_chapters += 1
                                error_msg = (
                                    f"Error organizing chapter {chapter.id}: {str(e)}"
                                )
    ...
    async def organize_user_library(
        ...
        async with AsyncSessionLocal() as db:
            ...
            result = await db.execute(
                select(MangaUserLibrary.manga_id).where(
