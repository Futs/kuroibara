...
from app.models.mangaupdates import DownloadClient, Download
...
class BaseDownloadClient(ABC):
    ...
    @abstractmethod
    async def remove_download(self, external_id: str, delete_data: bool = False) -> bool:
...
class QBittorrentClient(BaseDownloadClient):
    ...
    async def remove_download(self, external_id: str, delete_data: bool = False) -> bool:
        ...
        endpoint = "/api/v2/torrents/delete" if delete_data else "/api/v2/torrents/remove"
        ...
        async with self.session.post(url, data=data) as response:
            if response.status == 200:
                ...
                if result.get("status"):
                    ...
                    raise Exception(f"SABnzbd error: {result.get('error', 'Unknown error')}")
            else:
                ...
                for slot in queue.get("slots", []):
                    if slot.get("nzo_id") == external_id:
                        return {
                            ...
                            "downloaded_size": self._parse_size(slot.get("sizeleft", "0")),
                            "download_speed": self._parse_speed(slot.get("kbpersec", "0")),
            ...
            if response.status == 200:
                ...
                for slot in history.get("slots", []):
                    if slot.get("nzo_id") == external_id:
                        return {
                            "status": self._map_sab_status(slot.get("status", "")),
                            "progress": 100.0 if slot.get("status") == "Completed" else 0.0,
                            "total_size": self._parse_size(slot.get("size", "0")),
...
class DownloadClientService:
    ...
    async def get_client(self, client_config: DownloadClient) -> BaseDownloadClient:
        ...
        implementation = self.CLIENT_IMPLEMENTATIONS.get(client_config.implementation.lower())
        if not implementation:
            raise ValueError(f"Unsupported download client: {client_config.implementation}")
        
        return implementation(client_config)
    
    async def test_all_clients(self, db: AsyncSession) -> Dict[str, tuple[bool, Optional[str]]]:
    ...
    async def get_best_client(self, db: AsyncSession, client_type: str) -> Optional[DownloadClient]:
        ...
        result = await db.execute(
            ...
            .where(
                ...
                (DownloadClient.is_healthy == True)
            )
