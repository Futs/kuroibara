...
class ProviderMatcher:
    ...
    async def _search_provider(self, provider, mu_entry: MangaUpdatesEntry) -> List[Dict]:
    ...
    def _calculate_confidence(self, mu_entry: MangaUpdatesEntry, provider_result) -> float:
        """Calculate confidence score for a match."""
        # Title similarity (primary factor)
        title_similarity = SequenceMatcher(
            None, 
            mu_entry.title.lower(), 
            provider_result.title.lower()
        ).ratio()
        ...
        if mu_entry.alternative_titles:
            ...
            if alt_similarities:
                confidence += max(alt_similarities) * 0.2  # 20% weight for best alt title
        
        # Year similarity (if available)
        if (mu_entry.year and hasattr(provider_result, 'year') and 
            provider_result.year and abs(mu_entry.year - provider_result.year) <= 1):
...
class EnhancedSearchService:
    ...
    async def search(
        ...
        for mu_result in mu_results["results"]:
            try:
                ...
                # Check if in library
                if user_id:
                    enhanced_result.in_library, enhanced_result.library_manga = await self._check_library_status(
                        mu_entry, user_id, db
                    )
                
                # Find provider matches if requested
                if include_provider_matches:
                    enhanced_result.provider_matches = await self.provider_matcher.find_provider_matches(
                    ...
                    if enhanced_result.provider_matches:
                        enhanced_result.confidence_score = max(
                            match["confidence"] for match in enhanced_result.provider_matches
                        )
    ...
    async def _get_or_create_mu_entry(
        ...
        if existing_entry:
            ...
            if existing_entry.needs_refresh():
                async with mangaupdates_service.api as api:
                    await mangaupdates_service._update_entry_from_api(existing_entry, api, db)
    ...
    async def add_to_library_from_mu(
        ...
        mapping = MangaUpdatesMapping(
            ...
            confidence_score=selected_provider_match.get("confidence", 1.0) if selected_provider_match else 1.0,
