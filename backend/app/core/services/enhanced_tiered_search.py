...
class EnhancedTieredSearchService:
    ...
    async def refresh_entry(
        self, entry_id: str, db: AsyncSession, force_refresh: bool = False
    ) -> bool:
    ...
    async def _get_cached_results(
        self, query: str, db: AsyncSession, page: int, limit: int
    ...
    async def _store_search_results(
        self, metadata_results: List[UniversalMetadata], db: AsyncSession
    ...
    def _create_entry_from_metadata(
        self, metadata: UniversalMetadata
    ) -> UniversalMangaEntry:
    ...
    def _update_entry_from_metadata(
        self, entry: UniversalMangaEntry, metadata: UniversalMetadata
    ):
        ...
        entry.alternative_titles = (
            metadata.alternative_titles or entry.alternative_titles
        )
        ...
        entry.is_nsfw = (
            metadata.is_nsfw if metadata.is_nsfw is not None else entry.is_nsfw
        )
    ...
    async def _convert_to_search_result(
        ...
        return SearchResult(
            ...
            provider=(
                "enhanced_mangaupdates"
                if entry.source_indexer == "mangaupdates"
                else entry.source_indexer
            ),
    ...
    async def _check_library_status(
        self, entry: UniversalMangaEntry, user_id: Optional[str], db: AsyncSession
    ...
    async def _get_cross_references(
        self, entry: UniversalMangaEntry, db: AsyncSession
    ) -> Dict[str, Dict]:
    ...
    def _entry_to_dict(self, entry: UniversalMangaEntry) -> Dict:
        ...
        return {
            ...
            "data_completeness": entry.data_completeness,
            "last_refreshed": (
                entry.last_refreshed.isoformat() if entry.last_refreshed else None
            ),
        }
