...
class AnilistClient(BaseIntegrationClient):
    ...
    def __init__(self, client_id: Optional[str] = None, client_secret: Optional[str] = None):
        ...
        self.client_secret = client_secret or getattr(settings, 'ANILIST_CLIENT_SECRET', None)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status != 200:
                    ...
                    self.logger.error(f"Anilist auth failed - Status: {response.status}, Response: {error_text}")
