...
class KitsuClient(BaseIntegrationClient):
            ...
            async with ClientSession() as session:
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        logger.error(f"Kitsu authentication failed: {response.status} - {error_data}")
            ...
            async with ClientSession() as session:
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        error_data = await response.text()
                        logger.error(f"Kitsu token refresh failed: {response.status} - {error_data}")
    ...
    async def get_user_info(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            async with ClientSession() as session:
                async with session.get(f"{self.base_url}/users?filter[self]=true", headers=headers) as response:
                    if response.status == 200:
                        data = await response.json()
                        ...
                        error_data = await response.text()
                        logger.error(f"Kitsu get user info failed: {response.status} - {error_data}")
    ...
    async def get_user_manga_list(self, access_token: str, user_id: str) -> List[Dict[str, Any]]:
                    ...
                    async with session.get(url, headers=headers) as response:
                        if response.status == 200:
                            ...
                            for entry in data.get("data", []):
                                manga_info = self._extract_manga_from_entry(entry, data.get("included", []))
                                if manga_info:
                            ...
                            error_data = await response.text()
                            logger.error(f"Kitsu get manga list failed: {response.status} - {error_data}")
                            break
    ...
    def _extract_manga_from_entry(self, entry: Dict[str, Any], included: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                "title": manga_attributes.get("canonicalTitle") or manga_attributes.get("titles", {}).get("en"),
                ...
                "manga_data": {
                    ...
                    "poster_image": manga_attributes.get("posterImage", {}).get("large")
                }
            }
    ...
    async def update_manga_status(self, access_token: str, manga_id: str, status: str, progress: Optional[int] = None, rating: Optional[int] = None, notes: Optional[str] = None) -> bool:
        ...
        try:
            ...
            update_data = {
                "data": {
                    ...
                    "attributes": {
                        "status": self.map_status_to_kitsu(status)
                    }
                }
            }
    ...
    async def get_manga_list(self, access_token: str, status: Optional[str] = None, limit: int = 50, offset: int = 0) -> List[Dict[str, Any]]:
    ...
    async def search_manga(self, access_token: str, query: str, limit: int = 10) -> List[Dict[str, Any]]:
