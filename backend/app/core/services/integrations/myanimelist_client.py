...
class MyAnimeListClient(BaseIntegrationClient):
    ...
    def __init__(
        ...
        self.client_secret = client_secret or getattr(
            settings, "MAL_CLIENT_SECRET", None
        )
    ...
    async def get_manga_list(
        ...
        return ExternalMangaList(
            ...
            total_count=len(
                manga_list
            ),  # MAL doesn't provide total count in this endpoint
            has_next_page=len(manga_list)
    ...
    async def update_manga_status(
        ...
        data = {"status": self.map_status_to_mal(status)}
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.request(method, url, **kwargs) as response:
                if response.status not in [200, 201]:
                    error_text = await response.text()
                    raise Exception(
                        f"MyAnimeList API request failed: {response.status} - {error_text}"
                    )
