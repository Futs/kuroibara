...
class SyncService:
        ...
        async with AsyncSessionLocal() as db:
            ...
            try:
                ...
                if not await client.validate_token(integration.access_token):
                    ...
                    if integration.refresh_token:
                        try:
                            ...
                            logger.error(
                                f"Failed to refresh token for integration {integration_id}: {e}"
                            )
                ...
                if sync_direction in ["from_external", "bidirectional"]:
                    ...
                    sync_results["synced_manga_count"] += from_external_results[
                        "synced_count"
                    ]
                    sync_results["errors"].extend(from_external_results["errors"])
                    sync_results["created_mappings"] += from_external_results[
                        "created_mappings"
                    ]
                ...
                if sync_direction in ["to_external", "bidirectional"]:
                    ...
                    sync_results["updated_mappings"] += to_external_results[
                        "updated_count"
                    ]
    ...
    async def _sync_from_external(
        ...
        results = {"synced_count": 0, "errors": [], "created_mappings": 0}

        try:
            # Get external manga list
            external_manga_list = await client.get_manga_list(
                integration.access_token, limit=1000  # Get all manga
            )
            ...
            for external_manga in external_manga_list.manga:
                try:
                    ...
                    result = await db.execute(
                        select(ExternalMangaMapping).where(
                            and_(
                                ...
                                ExternalMangaMapping.external_manga_id
                                == external_manga.id,
                            )
                        )
                    )
                    ...
                    if not mapping:
                        ...
                        manga = await self._find_manga_by_title(
                            db, external_manga.title
                        )
                        ...
                        if manga:
                            ...
                            logger.info(
                                f"No matching manga found for external title: {external_manga.title}"
                            )
                            continue
                    ...
                    if mapping and mapping.manga_id:
                        ...
                        if (
                            ...
                            or integration.sync_ratings
                            or integration.sync_reading_progress
                        ):
    ...
    async def _sync_to_external(
        self, db: AsyncSession, integration: ExternalIntegration, client
    ) -> Dict[str, Any]:
            ...
            for mapping in mappings:
                try:
                    ...
                    if library_entry:
                        ...
                        chapters_read = len(
                            [p for p in progress_entries if p.is_completed]
                        )
                        ...
                        success = await client.update_manga_status(
                            ...
                            progress=(
                                chapters_read
                                if integration.sync_reading_progress
                                else None
                            ),
                            score=(
                                library_entry.rating
                                if integration.sync_ratings
                                else None
                            ),
    ...
    async def _find_manga_by_title(
        ...
        result = await db.execute(select(Manga).where(Manga.title.ilike(f"%{title}%")))
    ...
    async def _update_local_manga_status(
        ...
        if not library_entry:
            library_entry = MangaUserLibrary(
                user_id=user_id, manga_id=manga_id, status="reading"
            )
        ...
        if integration.sync_status and external_manga.status:
            client = self._get_client(integration)
            library_entry.status = client.map_status_from_external(
                external_manga.status
            )
