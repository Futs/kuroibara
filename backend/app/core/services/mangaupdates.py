...
class MangaUpdatesAPI:
    ...
    async def __aenter__(self):
        ...
        self.session = aiohttp.ClientSession(
            ...
            headers={"User-Agent": "Kuroibara/1.0 (https://github.com/Futs/kuroibara)"},
        )
        return self
    ...
    async def get_series_details(self, series_id: int) -> Optional[Dict]:
        ...
        logger.info(
            f"MangaUpdatesAPI.get_series_details called with series_id: {series_id}"
        )
        ...
        try:
            async with self.session.get(url) as response:
                ...
                if response.status == 200:
                    logger.info("Success response, parsing JSON...")
                    result = await response.json()
                    logger.info(f"JSON parsed successfully - type: {type(result)}")

                    if isinstance(result, dict):
                        logger.info(
                            f"Response is dict with keys: {list(result.keys())}"
                        )
                        ...
                        if "error" in result:
                            logger.error(
                                f"API returned error in response: {result['error']}"
                            )
                    else:
                    ...
                    return (
                        response_text  # Return the error text so we can see what it is
                    )
...
class MangaUpdatesService:
    ...
    async def search_and_create_entry(
        self, query: str, db: AsyncSession, auto_select_best: bool = True
    ...
    async def _create_entry_from_api(
        self, series_id: int, api: MangaUpdatesAPI, db: AsyncSession
    ) -> Optional[MangaUpdatesEntry]:
        ...
        try:
            ...
            if isinstance(details, dict):
                ...
                logger.info(
                    f"API response content (first 500 chars): {str(details)[:500]}"
                )
            ...
            logger.info("Calling _extract_entry_data with details")
            ...
            # Create entry
            logger.info("Creating MangaUpdatesEntry object")
            entry = MangaUpdatesEntry(**entry_data)
            logger.info("Adding entry to database session")
            db.add(entry)
            ...
            logger.info(
                f"Successfully created MangaUpdates entry for series {series_id}: {entry.title}"
            )
            return entry
    ...
    async def _update_entry_from_api(
        self, entry: MangaUpdatesEntry, api: MangaUpdatesAPI, db: AsyncSession
    ) -> None:
        ...
        await db.commit()
        logger.info(
            f"Updated MangaUpdates entry for series {entry.mu_series_id}: {entry.title}"
        )

    def _extract_entry_data(self, api_data: Dict) -> Dict:
        """Extract relevant data from MangaUpdates API response."""
        logger.info(
            f"_extract_entry_data called with type: {type(api_data)}, data: {api_data}"
        )
    ...
    def _extract_artists(self, data: Dict) -> Optional[List[Dict]]:
        ...
        for author_data in data.get("authors", []):
            if author_data.get("type") == "artist" and author_data.get("name"):
                artists.append({"name": author_data["name"], "type": "artist"})
    ...
    async def refresh_stale_entries(
        ...
        result = await db.execute(
            ...
            .where(
                (MangaUpdatesEntry.auto_refresh_enabled.is_(True))
                & (
                    (MangaUpdatesEntry.last_refreshed.is_(None))
