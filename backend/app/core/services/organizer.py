...
class MangaOrganizer:
    ...
    async def organize_chapter(
        ...
        try:
            ...
            should_preserve = preserve_original if preserve_original is not None else user.preserve_original_files
            ...
            if not self.ensure_directory_exists(organized_dir):
                result.add_error(f"Failed to create organized directory: {organized_dir}")
                return result
                ...
                if os.path.isdir(chapter.file_path):
                    ...
                    if not should_preserve:
                        try:
                            ...
                            logger.info(f"Removed original directory: {chapter.file_path}")
                        except Exception as e:
                            result.add_warning(f"Failed to remove original directory: {e}")
                    else:
                        # Move to raw storage
                        raw_base = self.get_raw_base_path(manga.id)
                        raw_path = os.path.join(raw_base, os.path.basename(chapter.file_path))
                        if self.safe_move_file(chapter.file_path, raw_path):
                            result.add_organized_file(raw_path)
                
                elif chapter.file_path.lower().endswith(('.cbz', '.cbr', '.zip', '.rar', '.7z')):
            ...
            logger.info(f"Successfully organized chapter {chapter.number} for manga {manga.title}")
