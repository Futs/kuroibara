...
class StorageRecoveryService:
    ...
    async def scan_storage_for_manga(
        ...
        if not os.path.exists(self.manga_storage_path):
            logger.warning(
                f"Manga storage path does not exist: {self.manga_storage_path}"
            )
            return []
        ...
        result = await db.execute(
            select(Manga.id)
            .join(MangaUserLibrary)
            .where(MangaUserLibrary.user_id == user_id)
        )
        ...
        try:
            for item in os.listdir(self.manga_storage_path):
                ...
                if os.path.exists(organized_path) and os.listdir(organized_path):
                    try:
                        manga_info = await self._extract_manga_info_from_structure(
                            organized_path
                        )
                            ...
                            manga_info.update(
                                {
                                    ...
                                    "storage_size": self._calculate_directory_size(
                                        organized_path
                                    ),
                                }
                            )
    ...
    async def _extract_manga_info_from_structure(
        ...
        try:
            ...
            manga_dirs = [
                d
                for d in os.listdir(organized_path)
            ...
            return {
                ...
                "has_volume_structure": len(
                    [v for v in volumes.keys() if v != "Direct"]
                )
                > 0,
            }
    ...
    def _scan_volume_for_chapters(self, volume_path: str) -> List[Dict]:
        ...
        return sorted(
            chapters, key=lambda x: self._natural_sort_key(x.get("number", "0"))
        )
    ...
    def _extract_chapter_info_from_filename(self, filename: str) -> Optional[Dict]:
        ...
        try:
            ...
            return {"number": number, "title": title, "filename": filename}
    ...
    async def recover_manga_to_database(
        ...
        try:
            ...
            manga = Manga(
                ...
                status=(
                    MangaStatus(metadata.get("status"))
                    if metadata and metadata.get("status")
                    else MangaStatus.UNKNOWN
                ),
                type=(
                    MangaType(metadata.get("type"))
            ...
            logger.info(
                f"Successfully recovered manga '{manga_title}' with ID {manga.id}"
            )
            return manga
    ...
    async def _create_chapters_from_organized_files(
        self, manga: Manga, user_id: UUID, db: AsyncSession
    ) -> None:
        """Create chapter database entries from organized files."""
        try:
            organized_path = os.path.join(
                get_manga_storage_path(manga.id), "organized", manga.title
            )
            ...
            for item in os.listdir(organized_path):
                ...
                if os.path.isdir(item_path):
                    ...
                    await self._create_chapters_from_volume(
                        manga, volume_name, item_path, user_id, db
                    )

                elif item.endswith(".cbz"):
                    # Direct chapter file
                    await self._create_chapter_from_file(
                        manga, None, item_path, user_id, db
                    )
        ...
        try:
            for item in os.listdir(volume_path):
                if item.endswith(".cbz"):
                    ...
                    await self._create_chapter_from_file(
                        manga, volume, chapter_path, user_id, db
                    )
