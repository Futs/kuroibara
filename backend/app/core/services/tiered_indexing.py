...
class BaseIndexer(ABC):
    ...
    def _get_headers(self) -> Dict[str, str]:
        ...
        return {"User-Agent": "Kuroibara/1.0 (https://github.com/Futs/kuroibara)"}
...
class MangaUpdatesIndexer(BaseIndexer):
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    logger.warning(
                        "MangaUpdates DDOS protection triggered (429). Backing off..."
                    )
                    ...
                    logger.error(
                        f"MangaUpdates search failed: {response.status} - {error_text[:200]}"
                    )
                    return []
        ...
        try:
            async with self.session.get(url) as response:
                if response.status == 200:
                    ...
                    logger.error(
                        f"Failed to get MangaUpdates details {source_id}: {response.status}"
                    )
                    return None
        except Exception as e:
    ...
    async def test_connection(self) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            url = f"{self.base_url}/series/search"
            payload = {"search": "test", "perpage": 1, "stype": "title"}
            headers = {
                "Content-Type": "application/json",
                "User-Agent": "Kuroibara/1.0 (Manga Library Manager; https://github.com/Futs/kuroibara)",
            }
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    return (
                        False,
                        "DDOS protection triggered (429) - this is normal protection, not rate limiting",
                    )
                else:
                    error_text = await response.text()
...
class MadaraDexIndexer(BaseIndexer):
        ...
        try:
            async with self.session.get(url) as response:
                if response.status == 200:
                    ...
                    logger.error(
                        f"Failed to get MadaraDex details {source_id}: {response.status}"
                    )
                    return None
        except Exception as e:
    ...
    def _parse_manga_item(self, item_soup) -> Optional[UniversalMetadata]:
        ...
        try:
            ...
            if not genres:
                genre_links = item_soup.find_all(
                    "a", href=lambda x: x and "/genre/" in x
                )
            ...
            # Extract rating
            rating = None
            rating_elem = item_soup.find(class_="rating") or item_soup.find(
                class_="score"
            )
            if rating_elem:
        ...
        for selector in chapter_selectors:
            ...
            if chapter_elem:
                ...
                chapter_match = re.search(
                    r"chapter\s*(\d+(?:\.\d+)?)", chapter_text.lower()
                )
                if chapter_match:
        ...
        try:
            async with self.session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    logger.error(
                        f"Failed to get MangaDex details {source_id}: {response.status}"
                    )
                    return None
        except Exception as e:
    ...
    def _extract_people(self, item: Dict, person_type: str) -> List[Dict[str, str]]:
        ...
        for rel in relationships:
            if rel.get("type") == person_type:
                ...
                if name:
                    people.append({"name": name, "role": person_type})
...
class TieredSearchService:
            ...
            try:
                async with indexer as idx:
                    ...
                    if results:
                        ...
                        if (
                            indexer.tier == IndexerTier.PRIMARY
    ...
    async def get_details(
        self, source_indexer: str, source_id: str
    ) -> Optional[UniversalMetadata]:
    ...
    async def cross_reference_manga(
        self, primary_result: UniversalMetadata, search_other_indexers: bool = True
    ...
    def _find_best_match(
        self, target: UniversalMetadata, candidates: List[UniversalMetadata]
    ) -> Optional[UniversalMetadata]:
    ...
    def _calculate_similarity_score(
        self, target: UniversalMetadata, candidate: UniversalMetadata
    ) -> float:
