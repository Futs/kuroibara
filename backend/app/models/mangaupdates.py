...
class UniversalMangaEntry(BaseModel):
    ...
    # Source information
    source_indexer = Column(
        String(50), nullable=False, index=True
    )  # mangaupdates, madaradx, mangadx
    ...
    manga_mappings = relationship(
        ...
        back_populates="universal_entry",
...
class UniversalMangaMapping(BaseModel):
    ...
    universal_entry_id = Column(
        UUID(as_uuid=True), ForeignKey("universal_manga_entries.id"), nullable=False
    )
    ...
    )  # "manual", "auto_title", "auto_fuzzy", etc.
    ...
    universal_entry = relationship(
        "UniversalMangaEntry", back_populates="manga_mappings"
    )
...
class CrossIndexerReference(BaseModel):
    ...
    match_method = Column(
        String(50), nullable=False
    )  # "title_exact", "title_fuzzy", "manual", etc.
    ...
    # Relationships
    universal_entry = relationship(
        "UniversalMangaEntry", back_populates="cross_references"
    )
...
class MangaUpdatesMapping(BaseModel):
    ...
    manga_id = Column(UUID(as_uuid=True), ForeignKey("manga.id"), nullable=False)
    mu_entry_id = Column(
        UUID(as_uuid=True), ForeignKey("mangaupdates_entries.id"), nullable=False
    )
...
class Download(BaseModel):
    ...
    client_id = Column(
        UUID(as_uuid=True), ForeignKey("download_clients.id"), nullable=True
    )
