...
class MangaMetadata(BaseModel):
    ...
    manga_id = Column(
        UUID(as_uuid=True), ForeignKey("manga.id"), nullable=False, unique=True
    )
...
class ChapterMetadata(BaseModel):
    ...
    chapter_id = Column(
        UUID(as_uuid=True), ForeignKey("chapter.id"), nullable=False, unique=True
    )
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
...
class OrganizationJob(BaseModel):
    ...
    job_type = Column(
        String(50), nullable=False
    )  # organize_manga, organize_library, convert_cbz, etc.
    job_status = Column(
