...
class BackupCreateRequest(BaseModel):
    ...
    include_storage: bool = Field(True, description="Whether to include storage files in backup")
...
class BackupRestoreResponse(BaseModel):
    ...
    warnings: List[str] = Field(default_factory=list, description="Important warnings about the restore")
...
class BackupScheduleInfo(BaseModel):
    ...
    monthly_enabled: bool = Field(..., description="Whether monthly backups are enabled")
    next_daily_backup: Optional[str] = Field(None, description="Next daily backup time")
    next_weekly_backup: Optional[str] = Field(None, description="Next weekly backup time")
    next_monthly_backup: Optional[str] = Field(None, description="Next monthly backup time")


class OrphanedStorageCheck(BaseModel):
    """Schema for orphaned storage check results."""
    
    has_orphaned_files: bool = Field(..., description="Whether orphaned files were found")
    orphaned_count: int = Field(..., description="Number of orphaned manga found")
    message: str = Field(..., description="Descriptive message")
    recovery_url: Optional[str] = Field(None, description="URL to recovery page if needed")
...
class BackupMetadata(BaseModel):
    ...
    created_at: str = Field(..., description="Creation timestamp")
    kuroibara_version: str = Field(..., description="Version of Kuroibara that created the backup")
    includes_storage: bool = Field(..., description="Whether storage files are included")
...
class BackupRestoreRequest(BaseModel):
    ...
    confirm_overwrite: bool = Field(..., description="Confirmation that user wants to overwrite current data")
...
class BackupDeleteRequest(BaseModel):
    ...
    confirm_delete: bool = Field(..., description="Confirmation that user wants to delete the backup")
...
class BackupValidationResult(BaseModel):
    ...
    metadata: Optional[BackupMetadata] = Field(None, description="Extracted backup metadata")
    estimated_restore_time: Optional[str] = Field(None, description="Estimated time for restore")
...
class BackupSettings(BaseModel):
    ...
    backup_path: str = Field("/app/backups", description="Path where backups are stored")
    include_storage_daily: bool = Field(False, description="Include storage in daily backups")
    include_storage_weekly: bool = Field(True, description="Include storage in weekly backups")
    include_storage_monthly: bool = Field(True, description="Include storage in monthly backups")
