...
class TorrentStatsSchema(BaseModel):
    ...
    average_download_speed: float = Field(
        ..., description="Average download speed (bytes/sec)"
    )
...
class TorrentQualityFilter(BaseModel):
    ...
    exclude_keywords: List[str] = Field(
        default_factory=list, description="Keywords to exclude from results"
    )
    include_keywords: List[str] = Field(
        default_factory=list, description="Keywords that must be present"
    )
...
class EnhancedTorrentSearchRequest(TorrentSearchRequest):
    ...
    auto_select_best: bool = Field(
        False, description="Automatically select best torrent"
    )
...
class TorrentClientStats(BaseModel):
    ...
    total_download_speed: int = Field(
        ..., description="Total download speed (bytes/sec)"
    )
