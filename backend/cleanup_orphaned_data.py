...
async def cleanup_orphaned_data():
    ...
    async for session in get_db():
        try:
            ...
            """))
            print(f"✅ Cleaned up {result.rowcount} orphaned manga_user_library_category entries")
            ...
            """))
            print(f"✅ Cleaned up {result.rowcount} orphaned external_manga_mappings entries")
            ...
            """))
            print(f"✅ Cleaned up {result.rowcount} orphaned reading_list_manga entries")
            ...
            if orphaned_manga:
                print(f"⚠️  Found {len(orphaned_manga)} manga not in any user's library:")
                for manga in orphaned_manga:
                    print(f"   - {manga.title} (ID: {manga.id}, Provider: {manga.provider})")
