...
def check_config_file_contents():
    ...
    if default_file.exists():
        ...
        nsfw_providers = ["manga18fx", "mangadna"]
        for provider_id in nsfw_providers:
            provider_config = next((p for p in default_configs if p["id"] == provider_id), None)
            if provider_config:
                print(f"  {provider_id}: supports_nsfw = {provider_config.get('supports_nsfw', 'NOT SET')}")
            else:
    ...
    # Check batch files
    batch_files = ["providers_batch1.json", "providers_batch2.json", "providers_batch3.json", "providers_batch4.json"]
    for batch_file in batch_files:
        ...
        if batch_path.exists():
            ...
            nsfw_providers = ["manga18fx", "mangadna"]
            for provider_id in nsfw_providers:
                provider_config = next((p for p in batch_configs if p["id"] == provider_id), None)
                if provider_config:
                    print(f"  {provider_id}: supports_nsfw = {provider_config.get('supports_nsfw', 'NOT SET')}")
