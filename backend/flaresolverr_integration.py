...
class CloudflareBypassProvider:
    ...
    def __init__(self, use_flaresolverr: bool = False, flaresolverr_url: str = "http://localhost:8191"):
        self.use_flaresolverr = use_flaresolverr
        self.flaresolverr_client = FlareSolverrClient(flaresolverr_url) if use_flaresolverr else None
        
    async def make_request(self, url: str, headers: Optional[Dict[str, str]] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with httpx.AsyncClient(timeout=30, follow_redirects=True) as client:
                ...
                if response.status_code in [403, 503, 521] or "cloudflare" in response.text.lower():
                    logger.info(f"Cloudflare protection detected for {url}, trying FlareSolverr")
                    
                    if self.use_flaresolverr and self.flaresolverr_client:
                        return await self.flaresolverr_client.get(url, headers)
                    else:
                        logger.warning(f"Cloudflare protection detected but FlareSolverr not enabled for {url}")
                        return None
