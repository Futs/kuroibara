...
async def test_hiperdex_provider():
    ...
    try:
        ...
        logger.info(
            f"Search returned {len(search_results)} results (total: {total}, has_more: {has_more})"
        )
    ...
    if "details" in locals() and details:
        ...
        try:
            ...
            if manga_url:
                chapters, total_chapters, has_more = await provider.get_chapters(
                    manga_url
                )
                logger.info(
                    f"Found {len(chapters)} chapters (total: {total_chapters}, has_more: {has_more})"
                )
        ...
        try:
            ...
            if chapter_url:
                ...
                pages = await provider.get_pages(
                    "", chapter_url
                )  # manga_id not needed for HiperDEX
...
async def test_javascript_provider_base():
    ...
    try:
        ...
        js_provider = JavaScriptProvider(
            name="TestJS", url="https://example.com", supports_nsfw=True
        )
