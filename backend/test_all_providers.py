...
class ProviderTester:
    ...
    async def test_url_accessibility(self, url: str, provider_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.timeout, follow_redirects=True) as client:
                ...
                result["redirected_url"] = str(response.url) if str(response.url) != url else None
    ...
    async def test_provider_search(self, provider, test_query: str = "naruto") -> Dict[str, Any]:
        ...
        try:
            search_results, total, has_more = await provider.search(test_query, page=1, limit=5)
    ...
    async def test_provider_metadata(self, provider, manga_id: str = None) -> Dict[str, Any]:
        ...
        if not manga_id:
            ...
            try:
                ...
                logger.warning(f"Error getting manga ID from search for {provider.name}: {e}")
        ...
        try:
            ...
            if metadata and isinstance(metadata, dict):
                ...
                result["metadata_sample"] = {
                    ...
                    "genres": metadata.get("genres", [])[:5] if isinstance(metadata.get("genres"), list) else []
                }
    ...
    async def test_image_accessibility(self, image_url: str, provider_name: str) -> Dict[str, Any]:
    ...
    async def test_provider_comprehensive(self, provider_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if base_url:
            result["url_test"] = await self.test_url_accessibility(base_url, provider_name)
        ...
        if provider:
            ...
            if (result["metadata_test"].get("metadata_sample", {}).get("cover_image")):
                ...
                result["cover_image_test"] = await self.test_image_accessibility(cover_url, provider_name)
    ...
    async def load_and_test_config_file(self, config_file: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i, provider_config in enumerate(providers):
                try:
                    logger.info(f"Testing provider {i+1}/{len(providers)}: {provider_config.get('name', 'Unknown')}")
                    ...
                    logger.error(f"Error testing provider {provider_config.get('name', 'Unknown')}: {e}")
    ...
    def save_results(self, results: Dict[str, Any], filename: str = "provider_test_results.json"):
