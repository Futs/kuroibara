...
def test_without_flaresolverr():
    ...
    cloudflare_providers = [p for p in providers if p['name'] in ['ReaperScans', 'Manhuaga', 'MangaFire']]
...
def main():
    ...
    print("📊 SUMMARY")
    print("=" * 60)
    print(f"Without FlareSolverr: {total_without} total, {cloudflare_without} Cloudflare")
    print(f"With FlareSolverr:    {total_with} total, {cloudflare_with} Cloudflare")
    print(f"Difference:           +{total_with - total_without} total, +{cloudflare_with - cloudflare_without} Cloudflare")
