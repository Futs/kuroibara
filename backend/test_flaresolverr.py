...
class FlareSolverrTester:
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"❌ FlareSolverr returned status {response.status_code}"
                    )
                    return False
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                response = await client.post(
                    ...
                    json={"cmd": "sessions.create", "session": "kuroibara_test"},
                )
                ...
                if response.status_code == 200:
                    ...
                    if data.get("status") == "ok":
                        ...
                        logger.info(
                            f"✅ Created FlareSolverr session: {self.session_id}"
                        )
                        return True
                    else:
                        logger.error(
                            f"❌ Session creation failed: {data.get('message')}"
                        )
                        return False
                else:
                    logger.error(
                        f"❌ Session creation returned status {response.status_code}"
                    )
                    return False
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30, follow_redirects=True) as client:
                ...
                if (
                    ...
                    result["error"] = (
                        f"Cloudflare protection detected (status: {response.status_code})"
                    )
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=120) as client:
                ...
                if response.status_code == 200:
                    ...
                    result["error"] = (
                        f"FlareSolverr API returned {response.status_code}"
                    )
    ...
    async def test_provider(self, name: str, url: str) -> Dict[str, Any]:
        ...
        if direct_result["success"]:
            logger.info(
                f"  ✅ Direct request successful ({direct_result['response_time']}s)"
            )
        else:
        ...
        if flaresolverr_result["success"]:
            logger.info(
                f"  ✅ FlareSolverr successful ({flaresolverr_result['response_time']}s, {flaresolverr_result['content_length']} bytes)"
            )
        else:
