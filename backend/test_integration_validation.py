...
async def test_database_tables():
    ...
    try:
        ...
        async with AsyncSessionLocal() as session:
            ...
            required_columns = [
                'id', 'created_at', 'updated_at', 'user_id', 'integration_type',
                'client_id', 'client_secret', 'access_token', 'refresh_token',
                'sync_enabled', 'last_sync_status'
            ]
...
async def test_api_endpoints_live():
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get('http://localhost:8000/api/v1/integrations/settings') as response:
                # Should return 401 (unauthorized) which means endpoint exists
                if response.status == 401:
                    print("  ✅ Integration settings endpoint accessible (requires auth)")
                else:
                    print(f"  ⚠️  Integration settings endpoint returned {response.status}")
