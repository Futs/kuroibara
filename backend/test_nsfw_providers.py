...
def test_provider_nsfw_configuration():
    ...
    for provider_id in nsfw_providers:
        provider_info = next((p for p in providers_info if p["id"] == provider_id), None)
        ...
        if provider_info:
            ...
            if not provider_info['supports_nsfw']:
                print(f"  ❌ ERROR: {provider_info['name']} should support NSFW but is configured as supports_nsfw=false")
                print(f"      This provider MUST be configured with supports_nsfw=true to properly tag NSFW content!")
                print(f"      CRITICAL: This means NSFW content will NOT be properly tagged or filtered!")
            else:
...
def main():
    ...
    try:
        ...
        print("  • Frontend should apply NSFW blur and filtering based on user settings")
