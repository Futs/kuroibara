...
class TestEnhancedTieredSearchService:
    ...
    @pytest.mark.asyncio
    async def test_cross_reference_creation(
        self, service, sample_universal_entry, db: AsyncSession
    ):
    ...
    def test_search_result_conversion(self, service, sample_universal_entry):
        ...
        with patch.object(
            service, "_check_library_status", side_effect=mock_check_library_status
        ):
            ...
            result_data = {
                ...
                "authors": [
                    author.get("name", "")
                    for author in (sample_universal_entry.authors or [])
                ],
    ...
    async def test_refresh_functionality(
        ...
        with patch.object(
            service.tiered_service, "get_details", return_value=updated_metadata
        ):
...
class TestDatabaseIntegration:
    ...
    async def test_manga_mapping_creation(self, db: AsyncSession):
        ...
        # Create manga
        manga = Manga(title="Local Manga", type="manga", status="ongoing")
        db.add(manga)
...
class TestErrorHandling:
    ...
    async def test_network_error_handling(self, db: AsyncSession):
        ...
        # Mock network error
        with patch.object(
            service.tiered_service, "search", side_effect=Exception("Network error")
        ):
            response = await service.search(
                query="Network Test", db=db, page=1, limit=10
            )
