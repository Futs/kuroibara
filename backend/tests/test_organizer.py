...
class TestCBZConverter:
    ...
    def test_get_image_files_from_directory(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch(
                "app.core.services.cbz_converter.is_image_file"
            ) as mock_is_image:
                mock_is_image.side_effect = lambda path: any(
                    path.endswith(ext) for ext in [".jpg", ".png", ".gif"]
                )
    ...
    def test_create_cbz_metadata(self):
        ...
        with patch("app.core.services.cbz_converter.get_image_dimensions") as mock_dims:
            ...
            metadata = self.converter.create_cbz_metadata(
                self.manga, self.chapter, image_files
            )
