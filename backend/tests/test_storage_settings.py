...
class TestStorageSettings:
    ...
    def test_user_model_has_storage_fields(self):
        ...
        assert 'storage_type' in user_columns, "storage_type column missing from User model"
        assert 'max_upload_size' in user_columns, "max_upload_size column missing from User model"
    ...
    def test_user_settings_schema_includes_storage_fields(self):
        ...
        assert 'storage_type' in schema_fields, "storage_type missing from UserSettings schema"
        assert 'max_upload_size' in schema_fields, "max_upload_size missing from UserSettings schema"
    ...
    def test_user_settings_update_schema_includes_storage_fields(self):
        ...
        assert 'storage_type' in update_fields, "storage_type missing from UserSettingsUpdate schema"
        assert 'max_upload_size' in update_fields, "max_upload_size missing from UserSettingsUpdate schema"
    ...
    def test_database_schema_has_storage_columns(self):
        ...
        with engine.connect() as conn:
            ...
            columns = {row[0]: {'type': row[1], 'default': row[2]} for row in result.fetchall()}
            
            assert 'storage_type' in columns, "storage_type column missing from database"
            assert 'max_upload_size' in columns, "max_upload_size column missing from database"
            
            # Check defaults
            assert "'local'" in columns['storage_type']['default'], "storage_type default should be 'local'"
            assert "'100MB'" in columns['max_upload_size']['default'], "max_upload_size default should be '100MB'"
