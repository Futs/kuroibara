...
const hasAllGenres = selectedGenres.every((genre) =>
  manga.genres.some(
    (mangaGenre) => mangaGenre.toLowerCase() === genre.toLowerCase(),
  ),
);
```
...
_Result: Ongoing Japanese action-fantasy manga with "demon" in title, safe content only_
...
_Result: Recently completed romantic school comedies from 2023_
...
_Result: NSFW Korean action-adventure series_
