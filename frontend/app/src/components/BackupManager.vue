  ...
  <div class="backup-manager">
    <div class="bg-white dark:bg-dark-800 shadow rounded-lg overflow-hidden">
      ...
      <div class="border-t border-gray-200 dark:border-dark-600">
        ...
        <!-- Status Messages -->
        <div v-if="error" class="px-4 py-3 bg-red-50 dark:bg-red-900 border-b border-red-200 dark:border-red-700">
          <div class="flex">
        ...
        <div v-if="successMessage" class="px-4 py-3 bg-green-50 dark:bg-green-900 border-b border-green-200 dark:border-green-700">
          <div class="flex">
            <div class="flex-shrink-0">
        ...
        <!-- Backup Statistics -->
        <div v-if="backupList" class="px-4 py-3 bg-gray-50 dark:bg-dark-700 border-b border-gray-200 dark:border-dark-600">
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
            <div>
              <span class="font-medium text-gray-900 dark:text-white">Total Backups:</span>
              <span class="ml-1 text-gray-600 dark:text-gray-300">{{ backupList.total_count }}</span>
            </div>
            <div>
              <span class="font-medium text-gray-900 dark:text-white">Total Size:</span>
              <span class="ml-1 text-gray-600 dark:text-gray-300">{{ formatFileSize(backupList.total_size) }}</span>
            </div>
            <div>
              <span class="font-medium text-gray-900 dark:text-white">Latest:</span>
              <span class="ml-1 text-gray-600 dark:text-gray-300">
                {{ backupList.backups.length > 0 ? formatDate(backupList.backups[0].created_at) : 'None' }}
              </span>
            </div>
          </div>
        </div>
            ...
            <div class="flex items-center justify-between">
              <div class="flex-1 min-w-0">
                <div class="flex items-center">
                  ...
                  <span v-if="backup.includes_storage" class="ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200">
                    Full Backup
                  </span>
                  <span v-else class="ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200">
                    Database Only
                  </span>
                </div>
                ...
                <div class="mt-1 flex items-center text-xs text-gray-500 dark:text-gray-400">
                  ...
                  <span v-if="backup.metadata">v{{ backup.metadata.kuroibara_version }}</span>
                </div>
              </div>
    ...
    <div v-if="showCreateModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" @click="showCreateModal = false">
      <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white dark:bg-dark-800" @click.stop>
        <div class="mt-3">
          <h3 class="text-lg font-medium text-gray-900 dark:text-white">Create Backup</h3>
          <div class="mt-4 space-y-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Backup Name (optional)</label>
              <input
...
const handleFileUpload = async (event) => {
  ...
  if (!confirm('This will restore the backup and overwrite all current data. Are you sure you want to continue?')) {
