<template>
  <div class="duplicate-detection bg-white dark:bg-dark-800 rounded-lg shadow-lg p-6">
    <div class="flex items-center justify-between mb-6">
      <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Duplicate Detection</h2>
    ...
    <div v-if="scanning" class="mb-6">
      <div class="flex items-center justify-between mb-2">
        <span class="text-sm text-gray-600 dark:text-gray-400">Scanning library...</span>
    ...
    <!-- Bulk Actions Panel -->
    <div v-if="showBulkActions && duplicates.length > 0" class="mb-6 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
      <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-3">Bulk Actions</h3>
    ...
    <!-- No Duplicates Found -->
    <div v-if="!scanning && duplicates.length === 0 && hasScanned" class="text-center py-8">
      <svg class="w-16 h-16 text-green-500 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>
      <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">No Duplicates Found</h3>
      <p class="text-gray-600 dark:text-gray-400">Your library appears to be clean of duplicates.</p>
    </div>
      ...
      >
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center">
            ...
            <span class="ml-2 px-2 py-1 text-xs bg-red-100 dark:bg-red-900/20 text-red-800 dark:text-red-200 rounded-full">
            ...
            <div class="flex items-start space-x-3">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400">
                  {{ item.manga.authors?.map(a => a.name).join(', ') || 'Unknown Author' }}
                </p>
        ...
        <div class="inline-block align-bottom bg-white dark:bg-dark-800 rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
          <div>
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
              This will merge the reading progress, bookmarks, and metadata from all duplicates into the primary item and delete the others.
            </p>
            ...
            <div class="space-y-2">
              ...
              <div class="space-y-2">
                ...
                >
                  ...
                  />
                  <div class="flex-1">
                    <div class="font-medium text-gray-900 dark:text-white">{{ item.manga.title }}</div>
...
const totalDuplicateItems = computed(() => {
  return duplicates.value.reduce((total, group) => total + group.items.length, 0);
});
...
const deleteSelectedDuplicates = async () => {
  ...
  if (confirm(`Are you sure you want to delete ${selectedDuplicates.value.size} selected items?`)) {
    try {
      ...
      duplicates.value = duplicates.value
        .map(group => ({
          ...group,
          items: group.items.filter(item => !selectedDuplicates.value.has(item.id))
        }))
...
const confirmMerge = async () => {
  ...
  try {
    ...
    const idsToDelete = itemsToMerge.filter(id => id !== primaryMergeItem.value);
    ...
    duplicates.value = duplicates.value.filter(group =>
      !group.items.some(item => itemsToMerge.includes(item.id))
    );
