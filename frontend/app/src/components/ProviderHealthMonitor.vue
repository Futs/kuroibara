<template>
  <div
    class="provider-health-monitor bg-white dark:bg-dark-800 rounded-lg shadow-lg p-6"
  >
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
      ...
      <div class="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg">
        <div class="flex items-center">
          <div class="text-yellow-600 dark:text-yellow-400 text-2xl mr-3">
            ⚠️
          </div>
          <div>
            <div
    ...
    <div class="space-y-4">
      ...
      >
            ...
            >
              <div class="flex items-center space-x-1">
                ...
                <span class="font-medium"
                  >{{ provider.uptime_percentage || 0 }}%</span
                >
              </div>
              <div class="flex items-center space-x-1">
                <span>Response:</span>
        ...
        <div
          v-if="showHealthHistory && getProviderHistory(provider.id).length > 0"
          class="mt-4"
        >
          <div
          ...
          <div class="flex items-end space-x-1 h-16">
            <div
              v-for="(point, index) in getProviderHistory(provider.id).slice(
                -48,
              )"
              :key="index"
              ...
              :style="{
                height: `${point.responseTime ? Math.min((point.responseTime / 1000) * 100, 100) : point.isHealthy ? 20 : 10}%`,
              }"
...
const healthStats = computed(() => {
  ...
  return {
    ...
    warning: providers.filter(
      (p) => p.consecutive_failures > 0 && p.consecutive_failures < 3,
    ).length,
