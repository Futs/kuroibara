<template>
  <div class="search-status-bar bg-white dark:bg-dark-800 border-b border-gray-200 dark:border-dark-600 px-4 py-3">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="text-sm text-gray-600 dark:text-gray-400">
          ...
          <span v-else-if="searchStore.results.length > 0">
            {{ searchStore.results.length }} of {{ searchStore.pagination.total }} results
        ...
        <!-- Performance Info -->
        <div v-if="searchStore.performance.response_time_ms" class="text-xs text-gray-500 dark:text-gray-500">
          {{ searchStore.performance.response_time_ms }}ms
          <span v-if="searchStore.performance.cached" class="text-green-600">(cached)</span>
          <span v-if="searchStore.performance.fallback" class="text-yellow-600">(fallback)</span>
        </div>
      </div>
      ...
      <div class="flex items-center gap-4">
        <!-- Source Indicators -->
        <div v-if="searchStore.sources.length > 0" class="flex items-center gap-2">
        ...
        <div class="flex items-center gap-2">
          <label class="text-xs text-gray-600 dark:text-gray-400">Enhanced:</label>
          <button
            ...
            :class="searchStore.useEnhancedSearch ? 'bg-primary-600' : 'bg-gray-200 dark:bg-gray-700'"
          >
            <span
              class="inline-block h-3 w-3 transform rounded-full bg-white transition-transform"
              :class="searchStore.useEnhancedSearch ? 'translate-x-5' : 'translate-x-1'"
            ></span>
          </button>
        </div>
    ...
    <div v-if="showHealthDetails" class="mt-3 pt-3 border-t border-gray-200 dark:border-dark-600">
        ...
        >
          <div class="flex items-center gap-2">
            <div 
              ...
              :class="indexer.status === 'healthy' ? 'bg-green-400' : 'bg-red-400'"
            ></div>
            <span class="text-sm font-medium">{{ formatSourceName(name) }}</span>
