    ...
    <div class="bg-white dark:bg-dark-800 shadow rounded-lg overflow-hidden">
      ...
      <div class="border-t border-gray-200 dark:border-dark-600">
        ...
        <div class="px-4 py-4 border-b border-gray-200 dark:border-dark-600">
          ...
          >
            <svg v-if="recovering" class="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
              <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            {{ recovering ? 'Recovering...' : `Recover All (${recoverableManga.length})` }}
          </button>
        </div>
        ...
        <div v-if="recoverableManga.length > 0" class="divide-y divide-gray-200 dark:divide-dark-600">
          <div
          ...
          >
            <div class="flex items-center justify-between">
              <div class="flex-1 min-w-0">
                <div class="flex items-center">
                  ...
                  <span v-if="manga.has_volume_structure" class="ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200">
                    Volumes
                  </span>
                </div>
            ...
            <div v-if="expandedManga === manga.storage_uuid" class="mt-4 pl-4 border-l-2 border-gray-200 dark:border-dark-600">
              <div v-for="(chapters, volumeName) in manga.volumes" :key="volumeName" class="mb-3">
                <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ volumeName }}</h4>
                <div class="mt-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1">
                  <div
                  ...
                  <div v-if="chapters.length > 6" class="text-xs text-gray-400 dark:text-gray-500">
            ...
            <div class="mt-2">
              ...
              >
                {{ expandedManga === manga.storage_uuid ? 'Hide Details' : 'Show Details' }}
              </button>
            </div>
          </div>
        </div>
        ...
        <div v-else-if="!loading && scanned" class="px-4 py-8 text-center">
          ...
          <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">No recoverable manga found</h3>
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
            All manga in storage appear to be properly registered in the database.
          </p>
        </div>
        ...
        <div v-else-if="!loading && !scanned" class="px-4 py-8 text-center">
          ...
          <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">Storage Recovery</h3>
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Click "Scan Storage" to search for manga that can be recovered from storage.
          </p>
        </div>
      </div>
    </div>
...
const scanStorage = async () => {
  ...
  try {
    ...
    if (recoverableManga.value.length === 0) {
      successMessage.value = 'No recoverable manga found. All manga appear to be properly registered.';
    } else {
...
const recoverSingle = async (manga) => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      recoverableManga.value = recoverableManga.value.filter(m => m.storage_uuid !== manga.storage_uuid);
    } else {
