...
describe("Reader Store", () => {
  ...
  describe("Achievements", () => {
    it("should check page-based achievements", () => {
      ...
      const pageAchievements = newAchievements.filter(
        (a) => a.requirement.type === "pages" && a.requirement.value <= 100,
      );
  ...
  describe("Typography and Display Options", () => {
    it("should update typography", () => {
      ...
      expect(mockSetItem).toHaveBeenCalledWith(
        "typography",
        JSON.stringify(store.settings.typography),
      );
    });
    ...
    it("should update display options", () => {
      ...
      expect(mockSetItem).toHaveBeenCalledWith(
        "displayOptions",
        JSON.stringify(store.settings.displayOptions),
      );
    });
  });
