...
export const ROLES = {
  ...
  GUEST: {
    ...
    name: "Guest",
    level: 10,
    permissions: ["content.browse", "content.preview"],
...
export const useSecurityStore = defineStore("security", {
  ...
  getters: {
    ...
    canViewContent: (state) => (content) => {
      ...
      const contentRating =
        CONTENT_RATINGS[content.rating] || CONTENT_RATINGS.G;
      ...
      if (
        content.tags &&
        content.tags.some((tag) => filter.blockedTags.includes(tag))
      ) {
        return false;
      }
      ...
      if (
        content.language &&
        !filter.allowedLanguages.includes(content.language)
      ) {
        return false;
      }
    ...
    getSecurityAlerts: (state) => {
      return state.securityEvents.filter(
