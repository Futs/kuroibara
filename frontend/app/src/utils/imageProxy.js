...
export function handleImageError(event, originalUrl, onRetry = null) {
  ...
  if (needsProxy(originalUrl) && !img.src.includes('/api/v1/providers/image-proxy')) {
    console.warn(`Original external image failed, trying proxy: ${originalUrl}`);
...
export function preloadImage(url) {
  return new Promise((resolve, reject) => {
    ...
    img.onerror = () => {
      ...
      if (needsProxy(url) && !img.src.includes('/api/v1/providers/image-proxy')) {
