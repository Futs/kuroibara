                ...
                >
                  ...
                  >
                    <svg
                      ...
                      xmlns="http://www.w3.org/2000/svg"
                      :fill="
                        isCurrentProviderFavorite ? 'currentColor' : 'none'
                      "
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
...
const enhancedProviders = computed(() => {
  return providers.value
    .map((provider) => {
      const prefInfo = providerPreferencesStore.getProviderDisplayInfo(
        provider.id,
      );
      return {
        ...provider,
...
const toggleProviderFavorite = async () => {
  ...
  const success = await providerPreferencesStore.toggleProviderFavorite(
    provider.value,
  );
  if (!success) {
