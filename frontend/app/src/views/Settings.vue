...
const loadLibraryStats = async () => {
  try {
    ...
    if (response.ok) {
      ...
      libraryStats.value = {
        totalManga: stats.total_manga || 0,
...
// Reorganize all media
const reorganizeAllMedia = async () => {
  ...
  try {
    ...
    const response = await fetch('/api/v1/organizer/reorganize-all', {
      ...
      body: JSON.stringify({
        ...
        chapter_naming_format: namingFormatChapter.value,
        preserve_original_files: preserveOriginalFiles.value,
        create_cbz_files: createCbzFiles.value,
      }),
    });
