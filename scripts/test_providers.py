...
async def test_provider(provider_name: str, db: AsyncSession, user_id: uuid.UUID) -> ProviderTestResult:
    ...
    try:
        ...
        for query in TEST_QUERIES:
            try:
                ...
                logger.warning(f"Search timeout for {provider_name} with query '{query}'")
                continue
        ...
        try:
            details_task = asyncio.create_task(provider.get_manga_details(result.test_manga_id))
        ...
        # Test 3: Get chapters
        try:
            chapters_task = asyncio.create_task(provider.get_chapters(result.test_manga_id, page=1, limit=5))
            chapters, total_chapters, has_next = await asyncio.wait_for(chapters_task, timeout=15.0)
...
async def main():
    ...
    async for db in get_db():
        ...
        other_to_test = [p for p in all_provider_names if p not in PRIORITY_PROVIDERS and p not in SKIP_PROVIDERS]

        # Combine lists
        provider_names = priority_to_test + other_to_test[:10]  # Limit to avoid too many tests

        results: List[ProviderTestResult] = []

        print(f"🔍 Testing {len(provider_names)} providers (skipping {len(SKIP_PROVIDERS)} problematic ones)...")
        ...
        for result in results:
            if result.search_success and result.chapters_success and result.download_success:
